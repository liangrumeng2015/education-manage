...
class Demo extends Component {
  ...
  cropImage = () =>{
    ...
    console.log(subjectPoint,'答案');
    this.props.dispatch({
        type:'manageTopicsModel/existImg',
        subjectAnswerPoint:subjectPoint
    })
  }
   // 初始化裁剪框
  _ready(event){  
  ...
  // 请求获取高度
  getTop = () =>{
    reqGetLatestSubject({paperId:1}).then(res=>{
      console.log('reqCnode请求结果',res)
      if(res.module){
        var needTop = res.module.leftDownPoint.split(')')[0].split(',')[1]   // 500
        console.log('needTop',needTop)
        needTop = parseInt(needTop);
        console.log('needTop',needTop)
        this.setState({
          currentIdx:parseInt(res.module.pageId)
        })
      }
    })
  }
  ...
  getTop = () =>{
    // 1 标准卷    2  答案卷
    reqGetLatestSubject({paperId:1,subjectType:2}).then(res=>{
      console.log('reqCnode请求结果',res)
      if(res.module){
        needTop = res.module.leftDownPoint.split(')')[0].split(',')[1]   // 500
        console.log('needTop111',needTop)
        needTop = parseInt(needTop);
        this.setState({
